...
class multi_step_mode_lif(torch.autograd.Function):
    ...
    def forward(ctx: _Any, x: torch.Tensor, u_init: torch.Tensor, u_threshold: torch.Tensor, u_rest: torch.Tensor, tau_m: torch.Tensor, fp_lif: _Callable, bp_lif: _Callable) -> torch.Tensor:
        ...
        ctx.fp_lif(ctx.time_steps, o, u, h, x, u_init, u_threshold, u_rest, tau_m)
    ...
    def backward(ctx: _Any, grad_o: torch.Tensor, grad_u_last: torch.Tensor) -> torch.Tensor:
        ...
        ctx.bp_lif(ctx.time_steps, grad_o, grad_u, grad_h, grad_x, grad_u_init, grad_tau_m, o, u, h, x, u_init, u_threshold, u_rest, tau_m)
...
class LIF(_LIF):
    ...
    def forward_steps(self, x: torch.Tensor) -> torch.Tensor:
        ...
        o, self.u = multi_step_mode_lif.apply(x, self.u, self.u_threshold, self.u_rest, self.tau_m, self.functions.fp_lif, self.functions.bp_lif)
        return o